...
from .models import (
    Student, StudentGuardian, StudentAcademicHistory, 
    DisciplinaryIncident, HealthRecord, AttendanceRecord, 
    StudentDocument
)
from .schemas import (
    StudentCreate, StudentUpdate, StudentSearchFilters,
    GuardianRelationshipCreate, DisciplinaryIncidentCreate,
    AttendanceRecordCreate, HealthRecordCreate, StudentDocumentCreate
)
...
class StudentCRUD:
    ...
    async def create_student_full_workflow(
        ...
        try:
            ...
            if student_data.current_class_id:
                await StudentCRUD._validate_class_capacity(db, student_data.current_class_id)
            ...
            encrypted_emergency_contacts = await StudentCRUD._encrypt_emergency_contacts(
            ...
            student = Student(
                ...
                home_language=student_data.home_language.value if student_data.home_language else None,
                ...
                postal_address=student_data.postal_address.dict() if student_data.postal_address else None,
                current_grade_level=student_data.current_grade_level,
                current_class_id=student_data.current_class_id,
                enrollment_date=student_data.enrollment_date,
                blood_type=student_data.blood_type.value if student_data.blood_type else None,
            ...
            await StudentCRUD._create_initial_academic_history(db, student.id, school_id)
            
            # 7. Log the creation activity
            await StudentCRUD._log_student_activity(
                db, student.id, created_by_user_id, "student_created",
                {"student_number": student_number, "grade": student_data.current_grade_level}
            )
            
            await db.commit()
            await db.refresh(student)
            
            logger.info(f"Student created successfully: {student_number} for school {school_id}")
            return student
    ...
    async def delete_student(
        ...
        try:
            ...
            if soft_delete:
                ...
                await StudentCRUD._log_student_activity(
                    db, student_id, deleted_by_user_id, "student_soft_deleted", 
                ...
                await StudentCRUD._log_student_activity(
                    db, student_id, deleted_by_user_id, "student_hard_deleted",
    ...
    async def _encrypt_medical_data(
        medical_conditions: List[Any], 
        allergies: List[Any]
    ) -> str:
        """Encrypt medical conditions and allergies."""
        medical_data = {
            "conditions": [condition.dict() if hasattr(condition, 'dict') else condition for condition in medical_conditions],
            "allergies": [allergy.dict() if hasattr(allergy, 'dict') else allergy for allergy in allergies]
        }
    ...
    async def _decrypt_student_sensitive_data(student: Student):
        ...
        if student.medical_conditions_encrypted:
            try:
                medical_data = decrypt_sensitive_data(student.medical_conditions_encrypted)
                student.decrypted_medical_conditions = medical_data.get("conditions", [])
                student.decrypted_allergies = medical_data.get("allergies", [])
            except Exception as e:
                logger.warning(f"Failed to decrypt medical data for student {student.id}: {str(e)}")
        ...
        if student.emergency_contacts_encrypted:
            try:
                ...
                logger.warning(f"Failed to decrypt emergency contacts for student {student.id}: {str(e)}")
    ...
    async def _log_student_activity(
        ...
        logger.info(f"Student activity: {action} for {student_id} by {user_id}, details: {details}")
...
class GuardianCRUD:
    ...
    async def create_guardian_relationship(
        ...
        try:
            ...
            logger.info(f"Guardian relationship created: {guardian_data.guardian_user_id} -> {student_id}")
            return relationship
    ...
    async def _ensure_single_primary_contact(db: AsyncSession, student_id: UUID):
        ...
        update_query = update(StudentGuardian).where(
            StudentGuardian.student_id == student_id
...
class DisciplinaryCRUD:
    ...
    async def create_incident(
        ...
        try:
            ...
            logger.info(f"Disciplinary incident created for student {incident_data.student_id}")
            return incident
    ...
    async def _update_student_disciplinary_points(
        ...
        update_query = update(Student).where(
            Student.id == student_id
        ).values(
            disciplinary_points=Student.disciplinary_points + points_to_add
        )
