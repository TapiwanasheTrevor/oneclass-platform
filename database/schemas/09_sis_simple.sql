...
-- Simplified Database Schema (without RLS policies)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS sis.students (
    ...
    -- Medical Information (Basic - no encryption in simple version)
    ...
    -- Emergency Contacts (Basic - no encryption in simple version)
    emergency_contacts TEXT NOT NULL, -- JSON string - minimum 2 contacts required
...
CREATE TABLE IF NOT EXISTS sis.student_guardians (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS sis.student_academic_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    student_id UUID NOT NULL REFERENCES sis.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS sis.disciplinary_incidents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    student_id UUID NOT NULL REFERENCES sis.students(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS sis.student_documents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    student_id UUID NOT NULL REFERENCES sis.students(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_students_zimsec_number ON sis.students(zimsec_number) WHERE zimsec_number IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_students_current_grade ON sis.students(current_grade_level);
CREATE INDEX IF NOT EXISTS idx_students_current_class ON sis.students(current_class_id) WHERE current_class_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_students_status ON sis.students(status);
CREATE INDEX IF NOT EXISTS idx_students_enrollment_date ON sis.students(enrollment_date);

-- Guardian relationships indexes
CREATE INDEX IF NOT EXISTS idx_student_guardians_student_id ON sis.student_guardians(student_id);
CREATE INDEX IF NOT EXISTS idx_student_guardians_guardian_id ON sis.student_guardians(guardian_user_id);
CREATE INDEX IF NOT EXISTS idx_student_guardians_primary ON sis.student_guardians(student_id, is_primary_contact);

-- Academic history indexes
CREATE INDEX IF NOT EXISTS idx_academic_history_student_year ON sis.student_academic_history(student_id, academic_year_id);
CREATE INDEX IF NOT EXISTS idx_academic_history_year ON sis.student_academic_history(academic_year_id);

-- Disciplinary incidents indexes
CREATE INDEX IF NOT EXISTS idx_disciplinary_student_id ON sis.disciplinary_incidents(student_id);
CREATE INDEX IF NOT EXISTS idx_disciplinary_date ON sis.disciplinary_incidents(incident_date);
CREATE INDEX IF NOT EXISTS idx_disciplinary_severity ON sis.disciplinary_incidents(severity);
CREATE INDEX IF NOT EXISTS idx_disciplinary_status ON sis.disciplinary_incidents(status);

-- Attendance records indexes
CREATE INDEX IF NOT EXISTS idx_attendance_student_date ON sis.attendance_records(student_id, attendance_date);
CREATE INDEX IF NOT EXISTS idx_attendance_date ON sis.attendance_records(attendance_date);
CREATE INDEX IF NOT EXISTS idx_attendance_status ON sis.attendance_records(status);

-- Documents indexes
CREATE INDEX IF NOT EXISTS idx_documents_student_id ON sis.student_documents(student_id);
CREATE INDEX IF NOT EXISTS idx_documents_type ON sis.student_documents(document_type);
CREATE INDEX IF NOT EXISTS idx_documents_verified ON sis.student_documents(is_verified);
...
COMMENT ON SCHEMA sis IS 'Student Information System - Simplified schema for development';
COMMENT ON TABLE sis.students IS 'Core student information';
